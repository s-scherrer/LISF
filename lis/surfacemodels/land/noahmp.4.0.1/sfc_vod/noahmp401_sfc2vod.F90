...
!
!EOP
  type(ESMF_Field)    :: laiField, relsm1field, relrzsmfield
  ...
  call ESMF_StateGet(sfcState,"Relative Soil Moisture Layer 1",relsm1field,rc=status)
  call LIS_verify(status)
  call ESMF_FieldGet(sm1field,localDE=0,farrayPtr=relsm1, rc=status)
  call LIS_verify(status)

  call ESMF_StateGet(sfcState,"Relative Root Zone Soil Moisture",relrzsmfield,rc=status)
  call LIS_verify(status)
  call ESMF_FieldGet(rzsmfield,localDE=0,farrayPtr=relrzsm, rc=status)
  call LIS_verify(status)
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
      ...
      relsm1(t) = (sm1 - smcwlt) / (smcref - smcwlt)
      relrzsm(t) = (rzsm - smcwlt) / (smcref - smcwlt)
