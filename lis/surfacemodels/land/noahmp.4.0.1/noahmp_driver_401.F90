...
subroutine noahmp_driver_401(n, ttile, itimestep, &  
     ...
     apar    , psn     , sav     , sag     , psav    , bsfpar  , & ! out Noah MP only
     wsfpar  , rssun   , rssha   ,                               & ! out Noah MP only        
...
! real, dimension(1,1) :: albedoinout
  ...
  real, dimension(1,1) :: bsfparout
  real, dimension(1,1) :: wsfparout
...
! z0 and znt should be passed to WRF, if coupled. - dmm
  ...
  savout(1,1)   = sav
  sagout(1,1)   = sag
  bsfparout(1,1)   = bsfpar
  wsfparout(1,1)   = wsfpar
  rssunout(1,1) = rssun
  rsshaout(1,1) = rssha
  bgapout(1,1)  = bgap
  wgapout(1,1)  = wgap
...
! albedo    = albedoinout(1,1)
  ...
  sag = sagout(1,1)
  psav = psavout(1,1)
  bsfpar = bsfparout(1,1)
  wsfpar = wsfparout(1,1)
  rssun = rssunout(1,1)
  rssha = rsshaout(1,1)
  bgap = bgapout(1,1)
  wgap = wgapout(1,1)
