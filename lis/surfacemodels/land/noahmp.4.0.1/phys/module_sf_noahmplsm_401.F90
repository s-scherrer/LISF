...
! output
  ...
  REAL                           , INTENT(OUT)   :: BSFPAR
  REAL                           , INTENT(OUT)   :: WSFPAR
...
! compute energy budget (momentum & energy fluxes and phase changes) 
    ...
    CALL ENERGY (parameters,ICE    ,VEGTYP ,IST    ,NSNOW  ,NSOIL  , & !in
		 ...
		 Z0WRF  ,                                         &
                 ...
                 SAV    ,SAG    ,PSAV   ,BSFPAR ,WSFPAR ,QMELT  , & !out
                 FSA    ,FSR    ,TAUX   ,                         & !out
...
! Solar radiation: absorbed & reflected by the ground and canopy
  ...
  CALL  RADIATION (parameters,VEGTYP  ,IST     ,ICE     ,NSOIL   , & !in 
                   ...
                   SAV     ,SAG     ,PSAV    ,BSFPAR  ,WSFPAR  , & !out
                   FSR     ,FSA     ,FSRV    , & 
                   FSRG    ,BGAP    ,WGAP    )            !out
...
! variables required to calculate FAPAR
  PSAV = CAD(1) + CAI(1)
  BSFPAR = FABD(1)
  WSFPAR = FABI(1)
...
! fraction of carbon into leaf versus nonleaf
     ...
     ALEAFPT = LOG(1. - LOG(0.2)/(0.01*MAX(MAXLAI,0.01))) / MAX(MAXLAI, 0.01)
     ALEAFPT = MIN(50.0, MAX(-100.0, ALEAFPT*XLAI))   ! to avoid overflow
     ALEAFPT = 0.01*(1. - EXP(ALEAFPT))*XLAI
     ALEAFPT = MIN(50.0, MAX(-100.0, ALEAFPT))
     LEAFPT = EXP(ALEAFPT)
