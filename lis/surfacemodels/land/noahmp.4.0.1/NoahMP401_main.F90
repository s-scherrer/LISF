...
!                                  tmp_fsh               , & ! out   - sensible heat flux [W/m2]
            ...
            if (NOAHMP401_struc(n)%reset_maxlai.eq.1) then
                if (tmp_hour.eq.0 .and. tmp_minute.eq.0) then
                    ...
                    NOAHMP401_struc(n)%noahmp401(t)%maximum_annual_lai = MAX(tmp_lai, NOAHMP401_struc(n)%noahmp401(t)%maximum_annual_lai)
                    if (tmp_month.eq.2 .and. tmp_day.eq.15 .and. NOAHMP401_struc(n)%noahmp401(t)%maximum_lai_nsamples > 150) then
                        ...
                        ! during the year
                        NOAHMP401_struc(n)%noahmp401(t)%maxlai = NOAHMP401_struc(n)%noahmp401(t)%maximum_annual_lai
                        NOAHMP401_struc(n)%noahmp401(t)%maximum_lai_nsamples = 0
                        NOAHMP401_struc(n)%noahmp401(t)%maximum_annual_lai = 0.0
                    end if
                end if
            end if
...
! on the soil (by the strict ALMA definition of ESoil). - David Mocko
           ...
           !                                  vlevel=1,unit="W m-2",direction="UP",surface_type=LIS_rc%lsm_index)
            ...
            ![ 60] output variable: apar (unit=W/m2). ***  photosynthesis energy by canopy
            ...
            ![ 62] output variable: fapar (unit=W/m2). ***  fration of absorbed photosynthesically active energy
            call LIS_diagnoseSurfaceOutputVar(n, t, LIS_MOC_FAPAR, value = NOAHMP401_struc(n)%noahmp401(t)%fapar, &
                                              vlevel=1, unit="-", direction="-", surface_type = LIS_rc%lsm_index)

            ![ 62] output variable: daily fapar (unit=-). ***  daily mean fration of absorbed photosynthesically active energy
