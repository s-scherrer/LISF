...
!BOC
     ...
     integer :: lhour       ! specific local output writing time (hour)
     integer :: lmin        ! specific local output writing time (min)
...
!    endif
    ...
    Call ESMF_ConfigFindLabel(modelSpecConfig, "PSAV:", rc = rc)
    Call get_moc_attributes(modelSpecConfig, LIS_histData(n)%head_lsm_list, &
         "PSAV", &
         "photosynthetically_active_solar_radiation_absorbed_by_vegetation",   &
         "photosynthetically active solar radiation absorbed by vegetation",rc)
    if ( rc == 1 ) then
        call register_dataEntry(LIS_MOC_LSM_COUNT, LIS_MOC_PSAV, &
            LIS_histData(n)%head_lsm_list,&
            n, 1, ntiles,(/"W/m2"/), 2, (/"IN ", "OUT"/),1,1,1,&
            model_patch=.true.)
    endif
